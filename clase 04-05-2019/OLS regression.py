...
def SCEDataFrame(DataFrameX, DataFrameY):
	...
	for i in range(len(DataFrameX)):
		result=result.append({"data SCE" : (DataFrameY[i]-(valueA+valueB*DataFrameX[i]))**2}, ignore_index=True)
	return result

def SCTDataFrame(DataFrameX, DataFrameY):
	Ymean=DataFrameY.sum()/len(DataFrameY)
	result=pd.DataFrame(columns=["data SCT"])
	for i in range(len(DataFrameX)):
		result=result.append({"data SCT" : (DataFrameY[i]-Ymean)**2}, ignore_index=True)
	return result

def SCRDataFrame(DataFrameX, DataFrameY):
	Ymean=DataFrameY.sum()/len(DataFrameY)
	valueA=calcA(DataFrameX, DataFrameY)
	valueB=calcB(DataFrameX, DataFrameY)

	result=pd.DataFrame(columns=["data SCR"])
	for i in range(len(DataFrameX)):
		result=result.append({"data SCR" : ((valueA+valueB*DataFrameX[i])-Ymean)**2}, ignore_index=True)
	return result
...
if __name__=="__main__":
	...
	if(len(keyList)==2):
		dataFrameSCE=SCEDataFrame(data[keyList[0]], data[keyList[-1]])
		valueSCE=dataFrameSCE.sum()
		print("SCE value = ", valueSCE.values[0])

		dataFrameSCT=SCTDataFrame(data[keyList[0]], data[keyList[-1]])
		valueSCT=dataFrameSCT.sum()
		print("SCT value = ", valueSCT.values[0])

		dataFrameSCR=SCRDataFrame(data[keyList[0]], data[keyList[-1]])
		valueSCR=dataFrameSCR.sum()
		print("SCR value = ", valueSCR.values[0])
	...
	if(rows>1 and cols>=1):
		...
		axs.set_title(keyList[0]+" vs "+keyList[-1])
		axs.scatter(data[keyList[0]], data[keyList[-1]], color="red")
		# for data param (third param) see also https://matplotlib.org/api/_as_gen/matplotlib.pyplot.plot.html
		axs.plot(data[keyList[0]], valueABList[0].get("valueA") + valueABList[0].get("valueB")*data[keyList[0]], linestyle="-", color="black")
		axs.grid()
		axs.set_xlabel(keyList[0])
		axs.set_ylabel(keyList[-1])
